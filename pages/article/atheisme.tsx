import type { NextPage } from 'next';
import Head from 'next/head';
import PageIllustration from '../../components/content/PageIllustration';
import Quote from '../../components/content/Quote';
import GenericPageLayout from '../../components/layouts/GenericPageLayouts';
import spectrumListStyles from '../../styles/custom-partials/spectrum-list.module.scss';

const AtheismCustomArticle: NextPage = () => {
    return (
        <>
            <Head>
                <title>L&apos;athéisme</title>
                <meta name="title" content="L'athéisme" />
                <meta name="description" content="L'athéisme, pourquoi et quand se désigner comme athée ?" />
                <meta property="og:type" content="website" />
                <meta property="og:url" content="https://blog.com/article/atheisme" />
                <meta property="og:title" content="L'athéisme" />
                <meta property="og:description" content="L'athéisme, pourquoi et quand se désigner comme athée ?" />
                <meta property="og:image" content="https://blog.com/illustrations/samuel-martins-3U7HcqkIbb4-unsplash.jpg" />
                <meta property="twitter:card" content="summary_large_image"/>
                <meta property="twitter:url" content="https://blog.com/article/atheisme"/>
                <meta property="twitter:title" content="L'athéisme"/>
                <meta property="twitter:description" content="L'athéisme, pourquoi et quand se désigner comme athée ?"/>
                <meta property="twitter:image" content="https://blog.com/illustrations/samuel-martins-3U7HcqkIbb4-unsplash.jpg"/>
            </Head>
            <GenericPageLayout>
                <article>
                    <header>
                        <time dateTime="2021-12-27">27 décembre 2021</time>
                        <h1>L&apos;athéisme, suis-je athée ?</h1>
                        <PageIllustration>
                            <img src="/illustrations/samuel-martins-3U7HcqkIbb4-unsplash.jpg" alt="homme agenouillé, yeux fermés, tête dans sa main. Une bible et ses lunettes posés à ses côtés. La position peut-être comprise aussi bien comme une prière, une réflexion intense, voir même un accablement face au chagrin." width="1920" height="1280" />
                            <p>Photo de <a href="https://unsplash.com/@samuelmartins7" target="_blank" rel="noopener noreferrer">Samuel Martins</a> sur <a href="https://unsplash.com" target="_blank" rel="noopener noreferrer">Unsplash</a></p>
                        </PageIllustration>
                    </header>
                    <p>Je compile ici une vision personnelle de l&apos;athéisme. De nombreuses années mon été nécessaire pour en arriver à ce constat, <i>en l&apos;absence de nouveaux faits pour abreuver cette réflexion personnelle</i>, j&apos;estime avoir atteint la conclusion naturelle du sujet.</p>
                    <p>Cet avis n&apos;engage que son auteur et n&apos;a pas pour but de discréditer qui que ce soit ou de bafouer une quelconque religion.</p>
                    <section>
                        <h2>Qu&apos;est-ce que l&apos;athéisme exactement ?</h2>
                        <p>Étymologiquement, &quot;athée&quot; provient du grec ancien &quot;átheos&quot;, et se traduit littéralement par &quot;sans dieu&quot;. Une analyse rapide nous permet de noter deux éléments importants:</p>
                        <ol>
                            <li>L&apos;athéiste c&apos;est celui <i>sans théisme</i>. On doit tout d&apos;abord noter qu&apos;ici rien n&apos;est affirmer, ni positivement, ni négativement, sur la notion de théisme elle-même.</li>
                            <li>Le mot <i>Sans</i> indique <i>une absence</i> ou <i>un manque</i>. On peut donc y voir un constat neutre ou négatif.<br/> Ces nuances sont légitimées par l&apos;usage du mot <i>athée</i> au cours de l&apos;histoire qui est passé d&apos;une insulte signifiant &quot;impie&quot; au sens philosophique neutre utilisé ensuite.
                                <Quote>
                                    <p>En Grec, atheos et atheotēs (…) atheos était un terme de censure sévère et une condamnation morale (…) il fallut attendre plus tard pour que le terme soit utilisé dans un sens philosophique.</p>
                                    <cite title="Atheism in Pagan Antiquity">Drachmann, AB Atheism in Pagan Antiquity</cite>
                                </Quote>
                            </li>
                        </ol>
                        <p>Pour la suite de l&apos;article j&apos;utiliserai le sens philosophique, c&apos;est-à-dire la version moderne où <i>sans</i> veux dire <i>absence</i> sans connotation péjorative. De plus, je partirai d&apos;une vision monothéiste de dieu même si les notions abordées restent valables dans le cas du polythéisme.</p>
                        <p>Désormais nous faisons face à l&apos;obstacle suivant, pour définir ce qu&apos;est <i>être sans théisme</i>, il nous faut définir la notion de théisme elle-même.</p>
                        <h3>Le théisme et le déisme</h3>
                        <p>Le théisme, et le déisme, sont la croyance en l&apos;existence d&apos;un être suprême. Les caractéristiques d&apos;un tel être suprême sont: une conscience omnisciente, éternel, totipotence, dont l&apos;existence ne nécessite pas de cause extérieure à lui-même, à l&apos;origine de la création de toutes choses.</p>
                        <Quote>
                            <p>Les théistes, proprement et strictement nommés comme tels, affirment qu&apos;un être éternel à la conscience et la compréhension parfaite existe. Cet être est cause de lui-même et de toute choses.</p>
                            <cite title="Collier&apos;s Encyclopedia">Ralph Cudworth (1617-1688), première définition connue du terme (traduction de l&apos;anglais par mes soins)</cite>
                        </Quote>
                        <p>Un <strong>théiste</strong> accepte les révélations et doctrines de sa foi religieuse comme divinement inspirées. Il croit que son dieu intervient directement dans les affaires humaines au travers de moyens surnaturels comme les miracles.</p>
                        <p>Le <strong>déiste</strong> est similaire à une différence majeure prêt, il rejette les révélations et tous les actes surnaturels qui pourraient être imputé à l&apos;être suprême au nom de la raison. Le déiste souscrit donc à une approche naturaliste et rationnelle, avec en toile de fond, un être suprême qui existe mais n&apos;intervient pas dans les affaires de l&apos;univers au-delà de sa création.</p>
                        <h3>Différentes formes d&apos;athéisme</h3>
                        <p>Si l&apos;on reprend la définition initiale: <i>l&apos;athéisme</i> est le fait d&apos;être <i>sans théisme</i>, c&apos;est-à-dire, finalement, sans affiliation à une des religions révélées.</p>
                        <p>En même temps, rien n&apos;indique qu&apos;on ne puisse être à la fois <i>athée et déiste</i>. Accepter la raison et la science comme unique moyen de connaître le monde, n&apos;implique pas de nier l&apos;existence d&apos;un être suprême. Ce cas de figure est d&apos;autant plus pertinent à une époque où de nombreuses personnes se revendiquent à la fois rationalistes et porteuses d&apos;une spiritualité personnelle.<br/><small>C&apos;est probablement la position la plus personnelle dans cet article. Je considère pour ma part que le déisme est compatible avec l&apos;athéisme.</small></p>
                        <p>Tout cela dénote le besoin d&apos;encore affiner notre définition d&apos;athéisme pour en séparer les différentes interprétations:</p>
                        <ol>
                            <li>Est <strong>athée déiste</strong> celui qui croit en même temps en un être suprême et en la raison. Il ne s&apos;associe avec aucune religion mais croit en un être suprême forcément compatible avec une vision non surnaturelle du monde.</li>
                            <li>Est <strong>agnostique</strong> celui qui considère que l&apos;existence d&apos;une entité suprême est une information inaccessible. Il refuse de se prononcer sur un domaine métaphysique qui par essence échappe au raisonnement naturaliste.</li>
                            <li>Est <strong>athée négatif (ou faible)</strong> celui qui considère l&apos;affirmation <i>&quot;Il est nécessaire qu&apos;au moins un être suprême existe&quot;</i> comme fausse. C&apos;est l&apos;athée qui considère qu&apos;un être suprême est une notion superflue pour expliquer notre univers, sans pour autant rejeter la possibilité d&apos;une telle entité.</li>
                            <li>Est <strong>athée positif (ou fort)</strong> celui qui considère l&apos;affirmation <i>&quot;Au moins un être suprême existe&quot;</i> comme fausse. C&apos;est l&apos;athéisme le plus fort, l&apos;existence même d&apos;un être suprême est rejetée.</li>
                        </ol>
                    </section>
                    <section>
                        <h2>A quel moment puis-je me dire athée ?</h2>
                        <p>Au regard de la définition d&apos;athéisme décrite en détail ci-dessus, la réponse variera suivant le sens retenu: absence de théisme, agnosticisme, athéisme négatif, athéisme positif.</p>
                        <p>Cela n&apos;arrange guère nos affaires. Pour simplifier l&apos;exercice, nous allons découper ce questionnement en deux parties complémentaires:</p>
                        <h3>La question réciproque, suis-je théiste ?</h3>
                        <p>Le plus petit dénominateur commun entre toutes les définitions d&apos;athée vue jusqu&apos;ici est le rejet de la croyance en un dieu d&apos;une religion révélée. Si je ne me reconnais dans aucun théisme, je suis athée. Jésus n&apos;est pas mon sauveur, je ne me soumets pas à Allah, Jéhovah très peu pour moi, Bouddha n&apos;en parlons pas, Odin, Osiris, Zeus, Amateratu, Vishnu, ...</p>
                        <p>Vous avez passé en revue tous les dieux de l&apos;humanité et aucun ne vous à paru plus véridique que son voisin ? Si oui alors c&apos;est officiel vous n&apos;êtes pas théiste, donc athéiste.</p>
                        <p>Finalement c&apos;était simple. Reste à savoir jusqu&apos;à quel point vous êtes athée, mais ça n&apos;obère pas du fait que oui vous êtes bel et bien athée.</p>
                        <h3>Le spectre de l&apos;athéisme</h3>
                        <p>Ici on en revient à la définition d&apos;un être suprême: une conscience omnisciente, éternelle, totipotente, dont l&apos;existence ne nécessite pas de cause extérieure à elle-même, qui est la cause originelle de l&apos;univers. <br/>Plus un tel être suprême vous semble plausible, nécessaire, certain, et moins votre athéisme sera fort sur le spectre.</p>
                        <ol className={ spectrumListStyles['list-container'] }>
                            <li>Un être suprême existe, <strong>j&apos;y crois avec certitude</strong>. Je suis <strong>athée déiste</strong>.</li>
                            <li><strong>Je ne sais pas</strong> si un être suprême existe, est nécessaire ou même plausible. Je suis <strong>agnostique</strong>.</li>
                            <li>Un être suprême <strong>n&apos;est pas nécessaire</strong> pour comprendre l&apos;univers, j&apos;applique le rasoir d&apos;Ockham. Par contre, je ne sais pas si une telle entité existe ou est plausible. Je suis <strong>athéiste négatif</strong>.</li>
                            <li>Un être suprême <strong>n&apos;existe pas</strong>, je rejette également la possibilité de son existence. Je suis <strong>athéiste positif</strong>.</li>
                        </ol>
                    </section>
                    <section>
                        <h2>L&apos;avis de l&apos;auteur</h2>
                        <p>Il est temps de passer à la pratique. Je vais appliquer à moi-même le cadre d&apos;analyse décrit précédemment.<br />Avant de démarrer je rappelle qu&apos;il s&apos;agit uniquement de ma propre situation, et que mes réponses ont variées au cours du temps, et changerons peut-être encore à l&apos;avenir. Je reste évasif à dessein sur mes raisonnements car mon but n&apos;est pas de convaincre du bien-fondé de mon avis mais de noter l&apos;état de ma réflexion à ce moment de ma vie. Je vous invite à cheminer par vous-même.</p>
                        <h3>Suis-je athée ?</h3>
                        <p><strong>Oui je suis athée.</strong> Je ne me reconnais dans aucune religion révélée. J&apos;ai toujours été intéressé par ce type de sujet, mais pour moi les religions revêtent un intérêt historique et anthropologique. Aucune doctrine ou texte sacré ne m&apos;a interpellé comme de nature divine, je crois en la méthode scientifique et la raison comme meilleure moyen de connaître le monde.<br/> Je considère les textes sacrés comme des œuvres de fiction d&apos;origine humaine.<br /> Un croyant sincère d&apos;une religion révélée (théiste) est, selon moi, une personne passionnée mais mal informée qui attribue par erreur ses expériences et ses émotions à un être suprême imaginaire.</p>
                        <p><strong>Suis-je athée déiste ? Non !</strong> Je ne crois pas en un être suprême tel que défini plus haut.</p>
                        <p><strong>Suis-je agnostique ? Non !</strong> Je pense que les connaissances scientifiques et la logique nous permettent de prendre des positions plus tranchées sur la question de l&apos;existence d&apos;un être suprême.</p>
                        <p><strong>Suis-je athée négatif ? Non !</strong> Pendant longtemps je l&apos;ai été, car bien que la notion d&apos;être suprême ne me semble pas nécessaire pour expliquer le monde, je ne voyais pas comment réfuter la possibilité même de l&apos;existence d&apos;une telle entité.<br/> Si une entité suprême décide de ne pas du tout interagir avec l&apos;univers naturel au-delà de sa création, comment confirmer ou infirmer qu&apos;elle existe ? Réponse: en prouvant qu&apos;elle est nécessaire à la création du monde ou en montrant qu&apos;une telle entité ne peut exister, ce qui nous amène au degré suivant.</p>
                        <p><strong>Suis-je athée positif ? Oui !</strong> Le facteur clef selon moi est la notion d&apos;omnipotence, qui est illogique car se contredisant elle-même. Je rejette donc la possibilité d&apos;un tel être suprême, pour autant c&apos;est affaire de définition et donc je ne puis rejeter la plausibilité d&apos;une entité aux capacité équivalente moins la totipotence. Dans ce cas je redeviendrai un athée négatif.</p>
                        <p>Et vous ? Êtes-vous athée ?</p>
                    </section>
                </article>           
            </GenericPageLayout>
        </>
    )
}

export default AtheismCustomArticle;
